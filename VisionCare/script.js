...
// --------- VisionCare Calibration WebSocket ---------

let ws;

// estimate screen size in inches
function getScreenSizeInInches() {
    const dpi = window.devicePixelRatio * 96; // standard browser DPI base
...
function startCalibration() {
    // IMPORTANT: update this port to whatever Python prints
    ws = new WebSocket("ws://127.0.0.1:54321"); // <-- CHANGE PORT

    ws.onopen = () => {
        console.log("Connected to Python server");

        const screenSize = getScreenSizeInInches().toFixed(2);
        console.log("Sending screen size:", screenSize);
    ...
    ws.onmessage = (event) => {
        ...
        if (event.data === "CALIBRATION_OK") {
            alert("Calibration complete. You may proceed.");
            ws.close();

            // redirect or unlock test
    ...
    ws.onerror = (err) => {
        ...
        alert("Could not connect to calibration server.");
    };
}

// Auto-start calibration when page loads
// window.addEventListener("load", startCalibration);
// Auto-start calibration ONLY on test page
window.addEventListener("load", () => {
    if (document.getElementById("calibration-step")) {
        startCalibration();
    }
});
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const output = document.getElementById("symptom-checker-output");
    ...
    btn.addEventListener("click", () => {
        ...
        if (!symptoms) {
            output.textContent = "Please describe your symptoms.";
            return;
        }

        const prompt = `
You are a virtual eye-care assistant.
...
"${symptoms}"
        ...
        callGroqApi(output, loading, prompt);
    });
});
